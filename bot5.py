...
async def extrair_texto_da_imagem(url_imagem: str) -> str:
    ...
    (DeepSeek não suporta imagens, por isso Gemini é obrigatório aqui)
    """
    try:
        async with aiohttp.ClientSession() as session:
...
async def gerar_fundo_calendario(mes: str, ano: int) -> Optional[io.BytesIO]:
    ...
    """
    prompts = {
        "Janeiro": "fundo azul inverno com flocos de neve suaves, estilo aquarela, tons pastel, textura suave, sem texto",
        "Fevereiro": "fundo romântico coração e flores, tons rosa e vermelho, estilo aquarela, sem texto",
        "Março": "fundo primavera com flores silvestres e folhas verdes, tons pastel, estilo aquarela, sem texto",
        "Abril": "fundo de Páscoa com ovos decorados e flores, tons lilás e amarelo, estilo aquarela, sem texto",
        "Maio": "fundo de campo de flores coloridas, céu azul claro, estilo aquarela, sem texto",
        "Junho": "fundo de praia, areia clara e mar azul, sol brilhante, estilo aquarela de verão, sem texto",
        "Julho": "fundo de férias de verão, palmeiras e sol, cores quentes, estilo tropical aquarela, sem texto",
        "Agosto": "fundo de pôr do sol na praia, tons laranja e rosa, estilo aquarela, sem texto",
        "Setembro": "fundo de folhas de outono caindo, tons laranja e dourado, estilo aquarela, sem texto",
        "Outubro": "fundo de Halloween com abóboras e folhas secas, tons laranja e roxo, estilo aquarela, sem texto",
        "Novembro": "fundo de outono profundo, tons castanho e vinho, estilo aquarela, sem texto",
        "Dezembro": "fundo de Natal com árvores e neve, tons vermelho e verde, estilo aquarela, sem texto",
    }
    
    prompt = prompts.get(mes, f"fundo abstrato suave para calendário de {mes}, estilo aquarela, tons pastel, sem texto")
...
def desenhar_calendario_leituras(mes: str, ano: int, imagem_fundo: Optional[io.BytesIO] = None) -> io.BytesIO:
    ...
    if not imagem_fundo:
        ...
        estacoes = {
            ...
            "Maio": "#f0f5e8", "Junho": "#fff8e0",
            "Julho": "#ffe0e0", "Agosto": "#f5e6d3",
            "Setembro": "#f0ebe0", "Outubro": "#f5e0d0",
            "Novembro": "#e8e0e8", "Dezembro": "#e0f0f5",
        }
        cor_fundo = estacoes.get(mes, "#fff8f1")
    ...
    draw.text((margem + 2, 105 + 2), "Metas guardadas pelo comando !meta", fill="#000000", font=fonte_rodape)
...
@bot.command(name="editar", help="Edita título ou autor de um livro.")
...
@bot.command(name="remover", help="Remove um livro do histórico (lidos), TBR e LCs.")
...
@bot.command(name="removerlc", help="Remove um livro de todas as leituras conjuntas (metas/lembretes).")
...
@bot.command(name="buscar", help="Busca livros no histórico por título ou autor.")
...
@bot.command(name="addletra", help="Adiciona uma letra ao desafio A-Z manualmente.")
...
@bot.command(name="reavaliar", help="Reavalia um livro já lido.")
...
@bot.command(name="avaliar", help="Avalia um livro específico ou o último lido.")
...
@bot.command(name="desabafar", help="Regista emoções, reações e conversas sobre um livro para a review.")
...
@bot.command(name="remtbrpos", help="Remove um livro da TBR por posição. Ex: !remtbrpos Junho 2")
async def remover_tbr_por_posicao(ctx: commands.Context, categoria: str, posicao: int):
    cat = normalizar_categoria(categoria)
...
@bot.command(name="limpartbr", help="Remove TODOS os livros de uma categoria. Ex: !limpartbr Junho")
async def limpar_tbr_categoria(ctx: commands.Context, categoria: str):
    cat = normalizar_categoria(categoria)
...
@bot.command(name="meta", help='Cria metas de leitura conjunta. Ex.: !meta Junho "Quarta Asa - Rebecca Yarros" dia 7 até cap. 10')
...
# ==============================================================================
# COMANDO: CALENDARIOLC (COM FUNDO IA)
# ==============================================================================

@bot.command(name="calendariolc", help="Cria uma imagem do calendário mensal das leituras conjuntas (com fundo IA opcional).")
async def calendario_leituras_conjuntas(ctx: commands.Context, mes: Optional[str] = None, modo: str = "comia"):
    if Image is None:
